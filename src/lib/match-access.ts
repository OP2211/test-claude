...
export function isFootballMatchReadOnly(match: Pick<Match, 'status' | 'kickoff'>): boolean {
...
export function isCricketMatchReadOnly(match: Pick<CricketMatch, 'status' | 'start'>): boolean {
  if (match.status === 'finished') return true;
